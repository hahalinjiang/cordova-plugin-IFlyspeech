...
//    BOOL isShowDialog = [command.arguments objectAtIndex:1];
    ...
    if ([isShowDialog isEqualToString:@"0"]) {
        ...
        [self.iflyRecognizerView start];
          //去除科大讯飞标识的方法
        if (self.iflyRecognizerView.subviews.count) {
            UIView *impV = [self.iflyRecognizerView.subviews objectAtIndex:0];
            for (UIView *v in impV.subviews) {
                if ([v isKindOfClass:[UILabel class]]) {
                    UILabel *lbl = (UILabel *)v;
                    if ([lbl.text containsString:@"语音识别能力由讯飞输入法提供"]) {
                        //                        lbl.text = @"点击结束";
                        v.hidden = YES;
                        break;
                    }
